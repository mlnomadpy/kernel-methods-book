...
<p class="lead">[[ch:kernels-and-rkhs]] built the theory: positive definite kernels are exactly inner products after an embedding \(\Phi : \mathcal{X} \to \mathcal{H}\), and each kernel carries its own function space, the RKHS. This chapter turns that theory into a way of computing. Two results do all the work. The <em>kernel trick</em> says that any algorithm which touches its data only through pairwise inner products can be run, unchanged, in the feature space of any p.d. kernel, even an infinite-dimensional one. The <em>representer theorem</em> says that a large class of learning problems posed over the entire RKHS actually have their solutions in an \(n\)-dimensional subspace spanned by the training points. Together they explain how kernel methods can be simultaneously nonlinear, nonparametric, and computationally concrete, and they give a general recipe, used throughout the rest of the book, for turning a linear algorithm into a kernel method.</p>
...
Often the function \(\Psi\) has the separable form
