...
This chapter inherits the feature-space construction from Chapter [[ch:kernels-and-rkhs|Positive Definite Kernels and RKHS]] and asks which computations survive when the coordinates are hidden.
...
[[fig:feature-lift]] lets you raise the lift by hand and watch the separating plane, which no line in the plane could match, appear as the two rings pull apart along the new dimension.
...
::: {.proof}
[Proof and scope]{.box-title}

By the Moore-Aronszajn theorem of [[ch:kernels-and-rkhs]], \(K(x,x')=\langle\Phi(x),\Phi(x')\rangle_{\mathcal H}\). Replace each inner-product query made by the algorithm with this equal scalar. Induction over the algorithm's operations shows that every subsequent scalar, coefficient, and decision is unchanged. The conclusion fails for an algorithm that inspects coordinates, exploits input-space sparsity, or applies a coordinatewise nonlinear operation before taking inner products; such an algorithm needs a separate derivation. \(\square\)
:::
...
def feature_distance(r, sigma):
    return np.sqrt(2 * (1 - np.exp(-(r**2) / (2 * sigma**2))))

assert feature_distance(0., 1.) == 0.
np.testing.assert_allclose(feature_distance(10., 1.), np.sqrt(2), rtol=1e-12)
```
:::::
...
xa, xb = np.array([1., 2.]), np.array([2., 1.])
explicit = phi(xa) @ phi(xb)
kernel = (xa @ xb) ** 2
distance2 = (xa @ xa) ** 2 + (xb @ xb) ** 2 - 2 * kernel
np.testing.assert_allclose([explicit, kernel, distance2], [16, 16, 18])
```
...
xstar = np.array([2., 1.])
prediction = ((X @ xstar) ** 2) @ alpha
np.testing.assert_allclose([*alpha, prediction, alpha @ K @ alpha],
                           [1, 2, 0, 6, 5])
```
...
The finite representer reduction hands its coefficient systems to Chapter [[ch:kernel-ridge-and-friends|Kernel Ridge Regression and Friends]], where the loss and regularizer determine the actual solve.
