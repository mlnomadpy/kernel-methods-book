...
The convolution and dynamic-programming constructions of [[ch:efficient-string-and-tree-kernels|Efficient String and Tree Kernels]] established that a structured kernel can sum products over compatible decompositions without listing every feature. Graph kernels inherit that recipe, but cycles destroy the simple left-to-right or rooted recursion. The chapter's hardness results and restricted substructure families explain exactly what must be surrendered to regain tractability.
...
d = np.array([[0, 1, 1, 1, 2], [1, 0, 2, 2, 1],
              [1, 2, 0, 2, 1], [1, 2, 2, 0, 1],
              [2, 1, 1, 1, 0]], dtype=float)
minimum = np.linalg.eigvalsh(np.exp(-0.2 * d))[0]
assert abs(minimum - (-0.028)) < 5e-4
```
::::
...
Counting graph fragments gives a deterministic feature map, yet many structured objects arrive with hidden alignments, states, or ancestors rather than a single observed decomposition. [[ch:generative-and-marginalization-kernels|Generative and Marginalization Kernels]] addresses that unresolved ambiguity by averaging feature evidence over latent explanations, using conditional independence and dynamic programming to retain positive definiteness.
