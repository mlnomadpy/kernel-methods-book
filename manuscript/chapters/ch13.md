...
<figcaption>Adding one observation adds an entire row and column: exact kernel storage grows quadratically, while bandwidth determines whether the resulting matrix has exploitable spectral structure.</figcaption>
</figure>
...
<figcaption>Random-feature error falls as more frequencies average the Bochner integral, but redraw variability remains visible at any fixed budget. The feature count must be chosen against downstream prediction error, not smooth appearance alone.</figcaption>
</figure>
...
The table becomes a decision only after the operating constraints are added. The next map puts data adaptivity on one axis and streaming or distribution pressure on the other. It is not a ranking: moving across the map changes which approximation error and which systems cost are acceptable.

<figure class="viz" data-figure="approximation-decision-map" data-alt="A two-axis decision map places exact or matrix-free methods, Nyström, random features, and structured kernel products according to spectral concentration and pressure from streaming, privacy, or distribution constraints.">
<figcaption>Nyström earns its advantage when the observed spectrum is concentrated and landmarks can be reused; random features earn theirs when a seed is easier to carry than data-dependent state. Exact and structured products remain competitive when matrix-vector products are cheap without global low rank.</figcaption>
</figure>
