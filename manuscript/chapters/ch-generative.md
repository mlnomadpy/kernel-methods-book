...
reviewers:
  ...
  pedagogical: null
  specialist: null
provenance: provenance/ch-generative.yml
...
Let \(\mathcal X\) be a finite or countable set. A symmetric joint distribution \(P(x,z)\) on the product \(\mathcal X\times\mathcal X\), meaning \(P(x,z)=P(z,x)\ge 0\) and \(\sum_{x,z}P(x,z)=1\), is a *P-kernel* on \(\mathcal X\) if it satisfies the finitely positive semidefinite property: for every finite sample \(x_1,\dots,x_n\) the matrix \(\big(P(x_i,x_j)\big)_{i,j}\) is positive semidefinite. One then sets \(\kappa(x,z)=P(x,z)\).
:::

Any kernel with finite total mass rescales to a P-kernel. If \(\kappa\ge 0\) and \(M=\sum_{x,z\in\mathcal X}\kappa(x,z)\lt\infty\), then \(P=\kappa/M\) is a symmetric distribution with the same Gram matrices up to the positive factor \(1/M\), hence a P-kernel. So the class is at least as rich as the nonnegative summable kernels. The converse fails, and it fails for the simplest possible reason: not every symmetric distribution is positive semidefinite.

:::: {.example #example-25-1}
[Example (a distribution that is not a kernel)]{.box-title}

On the two-element set \(\mathcal X=\{x_1,x_2\}\) put all the mass off the diagonal, \(P(x_1,x_2)=P(x_2,x_1)=\tfrac12\) and \(P(x_1,x_1)=P(x_2,x_2)=0\). The Gram matrix is
...
### Conditional independence and marginalization {#conditional-independence}
...
::: {.proposition #prop-25-2}
[Proposition (conditional independence gives a P-kernel)]{.box-title}
...
$$\sum_{i,j}c_i c_j\,\phi_m(x_i)\phi_m(x_j)=\Big(\sum_i c_i\,\phi_m(x_i)\Big)^2\ge 0,$$

which shows each \(m\)-term is positive semidefinite. The full kernel is the nonnegative combination \(\sum_m P_M(m)\,\phi_m(x)\phi_m(z)\) of these terms with weights \(P_M(m)\ge 0\), and a nonnegative combination of positive semidefinite kernels is positive semidefinite. Hence \(P(x,z)\) is a P-kernel. [\(\square\)]{.qed}
::::
...
## The fixed-length marginalization kernel {#fixed-length}

Start with the simplest hidden model for strings. Fix a length \(n\) and an alphabet \(\Sigma\), and suppose each symbol \(s_i\) of a string \(s=s_1\dots s_n\) is emitted independently from a hidden state \(h_i\) drawn from a state set \(A\). If the hidden states are themselves independent across positions, \(P(h)=\prod_{i=1}^{n}P(h_i)\), then the marginalization kernel sums the joint emission probability over all hidden strings \(h\in A^n\):

$$\kappa(s,t)=\sum_{h\in A^n}P(s\mid h)\,P(t\mid h)\,P(h)=\sum_{h\in A^n}\prod_{i=1}^{n}P(s_i\mid h_i)\,P(t_i\mid h_i)\,P(h_i).$$
...
$$\kappa(s,t)=\prod_{i=1}^{n}\Big(\sum_{a\in A}P(s_i\mid a)\,P(t_i\mid a)\,P(a)\Big).$$

Each factor is a single position's contribution, so the exponential sum over \(|A|^n\) hidden strings costs only \(O(n\,|A|)\). If \(P(\cdot\mid a)\) is a genuine emission distribution the whole kernel is also normalized: summing \(\kappa(s,t)\) over all pairs \((s,t)\in\Sigma^n\times\Sigma^n\) telescopes position by position to \(\prod_{i=1}^n\big(\sum_{s_i,t_i}P(s_i,t_i)\big)=\prod_{i=1}^n 1=1\), confirming it is a P-kernel and not merely a positive semidefinite function. This model is unrealistic, since real symbols are correlated along the sequence, but it fixes the pattern of marginalizing over sums of products that the next model exploits with genuine structure.

## The hidden Markov model kernel {#hmm-kernel}
...
$$P_M(h)=P_M(h_1)\,P_M(h_2\mid h_1)\cdots P_M(h_n\mid h_{n-1}),\qquad P(s,t\mid h)=\prod_{i=1}^{n}P(s_i\mid h_i)P(t_i\mid h_i).$$

The kernel is again \(\kappa(s,t)=\sum_{h\in A^n}P(s\mid h)P(t\mid h)P_M(h)\), and it is a P-kernel for the same conditional-independence reason. Now the sum no longer factors across positions, because a hidden state is tied to its neighbor, but the Markov structure means the coupling reaches back only one step, which is exactly the situation dynamic programming was built for.

Group the hidden paths by their final state. For \(k=1,\dots,n\) and \(a\in A\), let \(A^k_a=\{h\in A^k:h_k=a\}\) be the state paths of length \(k\) ending in \(a\), and define the subkernel

$$\kappa_{k,a}(s,t)=\sum_{h\in A^k_a}\prod_{i=1}^{k}P(s_i\mid h_i)\,P(t_i\mid h_i)\,P_M(h_i\mid h_{i-1}),$$

where the first factor uses the initial probability \(P_M(h_1)\). At length one there is a single path, so \(\kappa_{1,a}(s,t)=P(s_1\mid a)P(t_1\mid a)P_M(a)\). To extend a path to length \(k+1\) ending in \(a\), we append the state \(a\) to some path ending in \(b\) at length \(k\), pay the transition \(P_M(a\mid b)\) and the emission of position \(k+1\), and sum over the predecessor \(b\):

$$\kappa_{k+1,a}(s,t)=P(s_{k+1}\mid a)\,P(t_{k+1}\mid a)\sum_{b\in A}P_M(a\mid b)\,\kappa_{k,b}(s,t).$$
...
:::: {.algorithm #algo-25-1}
[Algorithm (fixed-length HMM marginalization kernel)]{.box-title}

::: algo-io
[Input]{.algo-lab} Equal-length strings \(s,t\in\Sigma^n\); state set \(A\); emission \(P(\sigma\mid a)\); initial \(P_M(a)\); transition \(P_M(a\mid b)\).

[Output]{.algo-lab} The marginalization kernel \(\kappa(s,t)=\sum_{h\in A^n}P(s\mid h)P(t\mid h)P_M(h)\).
:::

1.  For each \(a\in A\), initialize \(\kappa_{1,a}\leftarrow P(s_1\mid a)\,P(t_1\mid a)\,P_M(a)\).
2.  For \(k=1,\dots,n-1\) and each \(a\in A\), set \(\kappa_{k+1,a}\leftarrow P(s_{k+1}\mid a)\,P(t_{k+1}\mid a)\sum_{b\in A}P_M(a\mid b)\,\kappa_{k,b}\), overwriting the length-\(k\) table.
...
[Example (marginalization kernel under a 2-state HMM)]{.box-title}

:::: wex
::: wex-setup
Alphabet \(\Sigma=\{A,B\}\), hidden states \(\{1,2\}\). Emissions \(P(A\mid 1)=0.8,\ P(B\mid 1)=0.2,\ P(A\mid 2)=0.3,\ P(B\mid 2)=0.7\). Initial \(P_M(1)=0.6,\ P_M(2)=0.4\). Transitions \(P_M(1\mid 1)=0.7,\ P_M(2\mid 1)=0.3,\ P_M(1\mid 2)=0.4,\ P_M(2\mid 2)=0.6\). Compare \(s=\texttt{AB}\) and \(t=\texttt{AA}\), so \(n=2\).
:::

1.  [Enumerate the four hidden paths.]{.wex-op} Each term is \(P(s\mid h)P(t\mid h)P_M(h)\) with \(P_M(h)=P_M(h_1)P_M(h_2\mid h_1)\). Path \((1,1)\): \(0.16\cdot 0.64\cdot 0.42=0.043008\). Path \((1,2)\): \(0.56\cdot 0.24\cdot 0.18=0.024192\). Path \((2,1)\): \(0.06\cdot 0.24\cdot 0.16=0.002304\). Path \((2,2)\): \(0.21\cdot 0.09\cdot 0.24=0.004536\).
2.  [Sum the joint probabilities.]{.wex-op} \(\kappa(s,t)=0.043008+0.024192+0.002304+0.004536=0.074040\).
3.  [Base of the recursion.]{.wex-op} With \(s_1=t_1=A\): \(\kappa_{1,1}=0.8\cdot 0.8\cdot 0.6=0.384000\) and \(\kappa_{1,2}=0.3\cdot 0.3\cdot 0.4=0.036000\).
4.  [Advance one position.]{.wex-op} With \(s_2=B,\ t_2=A\): \(\kappa_{2,1}=P(B\mid 1)P(A\mid 1)\,[P_M(1\mid 1)\kappa_{1,1}+P_M(1\mid 2)\kappa_{1,2}]=0.16\,(0.2688+0.0144)=0.045312\), and \(\kappa_{2,2}=P(B\mid 2)P(A\mid 2)\,[P_M(2\mid 1)\kappa_{1,1}+P_M(2\mid 2)\kappa_{1,2}]=0.21\,(0.1152+0.0216)=0.028728\).
5.  [Sum over the final state.]{.wex-op} \(\kappa(s,t)=\kappa_{2,1}+\kappa_{2,2}=0.045312+0.028728=0.074040\).
...
Sequences in biology rarely share a length. Two homologous genes differ not only by substitutions but by insertions and deletions, the gaps of a pairwise alignment, so a kernel that insists on equal lengths and a shared position index cannot compare them. The remedy is to let the hidden model emit to the two strings at different rates. Augment the alphabet with the empty symbol \(\varepsilon\); a hidden state may then emit a real symbol to one string while emitting \(\varepsilon\), that is nothing, to the other. A state that emits \((s_i,\varepsilon)\) advances only the first string, modeling a deletion; \((\varepsilon,t_j)\) advances only the second, an insertion; and \((s_i,t_j)\) advances both, a match or substitution. Because emissions no longer line up with positions, the states form a general Markov process, a probabilistic finite-state automaton with transition matrix \(M_{ab}=P_M(b\mid a)\) and a distinguished initial state \(a_I\).

:::: {.definition #def-25-3}
[Definition (pair hidden Markov model, Durbin et al. 1998)]{.box-title}

A *pair data model* is a model whose states emit ordered pairs of data items: each state \(h\) carries a distribution \(P_h(x_1,x_2)=P((x_1,x_2)\mid h)\) over pairs. A *pair hidden Markov model* is a hidden Markov model whose states emit ordered pairs. Its trajectories \(h=(h_0=a_I,h_1,\dots,h_N)\) begin at the initial state, and the joint emission of two strings is

$$P(s,t)=\sum_{h}P(s,t\mid h)\,P_M(h),\qquad P_M(h)=\prod_{i}P_M(h_i\mid h_{i-1}).$$
::::

After a standard preprocessing step that removes states emitting the doubly empty pair \((\varepsilon,\varepsilon)\), which would consume no input and stall the recursion, the model can be marginalized by dynamic programming over the two independent position indices, exactly as the string kernels of [[ch:string-kernels|the string-kernels chapter]] carry a table indexed by prefixes of each string. Let \(A_a=\{h\in\hat M:h_{|h|}=a,\ h_0=a_I\}\) be the trajectories ending in state \(a\), and define the subkernel over the prefixes \(s(1{:}i)\) and \(t(1{:}j)\):

$$\kappa_{i,j,a}(s,t)=\sum_{h\in A_a}\hat P\big(s(1{:}i),t(1{:}j)\mid h\big)\,P_{\hat M}(h).$$
...
$$\kappa_{i,j,a}(s,t)=\sum_{b\in A}P_M(a\mid b)\Big[\,P\big((s_i,\varepsilon)\mid a\big)\,\kappa_{i-1,j,b}+P\big((\varepsilon,t_j)\mid a\big)\,\kappa_{i,j-1,b}+P\big((s_i,t_j)\mid a\big)\,\kappa_{i-1,j-1,b}\,\Big].$$
...
:::: {.algorithm #algo-25-2}
[Algorithm (pair HMM marginalization kernel)]{.box-title}

::: algo-io
[Input]{.algo-lab} Strings \(s,t\) of lengths \(|s|,|t|\); state set \(A\) with initial \(a_I\); transitions \(P_M(a\mid b)\); pair emissions \(P((s_i,\varepsilon)\mid a),\ P((\varepsilon,t_j)\mid a),\ P((s_i,t_j)\mid a)\).

[Output]{.algo-lab} The pair HMM kernel \(\kappa(s,t)=\sum_{h}P(s\mid h)P(t\mid h)P_M(h)\).
:::

1.  Initialize \(\kappa_{0,0,a_I}\leftarrow 1\) and \(\kappa_{0,0,a}\leftarrow 0\) for \(a\neq a_I\).
2.  For \(i=0,\dots,|s|\) and \(j=0,\dots,|t|\) in order (skipping \(i=j=0\)) and each \(a\in A\), set \(\kappa_{i,j,a}\leftarrow \sum_{b}P_M(a\mid b)\big[P((s_i,\varepsilon)\mid a)\,\kappa_{i-1,j,b}+P((\varepsilon,t_j)\mid a)\,\kappa_{i,j-1,b}+P((s_i,t_j)\mid a)\,\kappa_{i-1,j-1,b}\big]\), treating out-of-range subkernels as \(0\).
...
## The hidden tree model kernel {#hidden-tree}
...
where \(h\) now ranges over labelings of the internal nodes \(v_0,\dots,v_N\) with states from \(A\), the root \(v_0\) drawn from a prior \(P_M(a)\), and each child \(\mathrm{ch}_j(v)\) in state \(b\) with probability \(P_j(b\mid a)\) given its parent's state \(a\). The leaves are pinned to the observed symbols.
...
$$\kappa_{v,a}(s,t)=\sum_{h\in T_a(v)}P(s\mid h)\,P(t\mid h)\,P_M(h),$$
...
$$\kappa_{v,a}(s,t)=\prod_{j=1}^{\mathrm{child}(a)}\Big(\sum_{b\in A}P_j(b\mid a)\,\kappa_{\mathrm{ch}_j(v),b}(s,t)\Big).$$
...
[Example (binary tree, binary states)]{.box-title}
...
The intuition is a gradient one. Suppose we hold a fitted model \(P_\theta\) and consider adapting its parameters to raise the likelihood of a new point \(x\). A point already typical of the model needs almost no adjustment, so its likelihood gradient is near zero; an unusual point pulls hard on whichever parameters are responsible for its oddity, giving a large gradient in those directions. The gradient of the log-likelihood is therefore a fingerprint of \(x\) relative to the model, of fixed dimension no matter the size of \(x\), and comparing two fingerprints compares how the two points would deform the model.

:::::: {.definition #def-25-4}
[Definition (Fisher score and Fisher information, Jaakkola and Haussler 1999)]{.box-title}

Let \(\{P_\theta:\theta\in\mathbb R^N\}\) be a smooth parametric model and fix a setting \(\theta_0\), for instance the maximum-likelihood estimate on a training set. The *Fisher score* of an example \(x\) is the gradient of the log-likelihood,

$$g(\theta_0,x)=\nabla_\theta\log L_\theta(x)\Big|_{\theta=\theta_0}=\Big(\tfrac{\partial}{\partial\theta_i}\log L_\theta(x)\Big)_{i=1}^{N},$$

one coordinate per parameter. The *Fisher information matrix* is the covariance of the score under the model,

$$I_M=\mathbb E\big[g(\theta_0,x)\,g(\theta_0,x)^\top\big],$$

the expectation taken over \(x\) generated by \(P_{\theta_0}\). The *Fisher kernel* is the whitened inner product of scores,

$$K(x,z)=g(\theta_0,x)^\top I_M^{-1}\,g(\theta_0,z).$$
::::::

The whitening by \(I_M^{-1}\) is not cosmetic. Under the model the expected score vanishes at every parameter setting, \(\mathbb E_{x\sim P_{\theta_0}}[g(\theta_0,x)]=0\), so \(I_M\) is exactly the covariance of the score, and multiplying by \(I_M^{-1}\) rotates and rescales the score coordinates into directions that are uncorrelated and of unit variance. This makes the kernel a Mahalanobis inner product that does not depend on how the model happens to be parametrized: the Fisher kernel is invariant under smooth reparametrization, which the general construction in [[ch:kernel-families|the kernel-families chapter]] proves. In practice \(I_M\) is costly and often nearly singular, so it is routinely replaced by the identity, in which case the Fisher kernel is the plain dot product of scores and the feature map \(\Phi(x)=g(\theta_0,x)\) is stored directly as a fixed-length vector, the *Fisher vector*. The following example computes such a score vector by hand for the smallest nontrivial model.

::::: {.example #example-25-4}
[Example (Fisher score for a 2-symbol iid model)]{.box-title}

:::: wex
::: wex-setup
Model: symbols drawn iid from \(\Sigma=\{A,B\}\) with unnormalized weights \(\theta=(\theta_A,\theta_B)\) and \(P(\sigma\mid\theta)=\theta_\sigma/(\theta_A+\theta_B)\). For a string \(s\) of length \(n\) with counts \((n_A,n_B)\), the log-likelihood is \(\log L(s)=\sum_\sigma n_\sigma\log\theta_\sigma-n\log(\theta_A+\theta_B)\). Evaluate at the uniform setting \(\theta_A=\theta_B=1\) (so \(Z=2\), \(P=\tfrac12\)). Compare \(s=\texttt{AAB}\), \(t=\texttt{ABB}\), and \(u=\texttt{AAA}\), all of length \(3\).
:::

1.  [Differentiate the log-likelihood.]{.wex-op} \(\dfrac{\partial}{\partial\theta_\sigma}\log L=\dfrac{n_\sigma}{\theta_\sigma}-\dfrac{n}{Z}\). At \(\theta_A=\theta_B=1,\ Z=2\) this is \(g(s)_\sigma=n_\sigma-\tfrac n2\), so the score vector is \(g(s)=(n_A-\tfrac n2,\ n_B-\tfrac n2)\).
2.  [Evaluate the score vectors.]{.wex-op} \(g(s)=(2-1.5,\,1-1.5)=(0.5,-0.5)\); \(g(t)=(1-1.5,\,2-1.5)=(-0.5,0.5)\); \(g(u)=(3-1.5,\,0-1.5)=(1.5,-1.5)\). Each obeys \(g_A+g_B=0\), the centering that projects onto the direction perpendicular to the all-ones vector.
3.  [Form the Fisher kernel with \(I=\mathrm{Id}\).]{.wex-op} \(K(s,t)=g(s)\cdot g(t)=(0.5)(-0.5)+(-0.5)(0.5)=-0.5\), while \(K(s,s)=0.25+0.25=0.5\) and \(K(t,t)=0.5\). The mismatched pair \((s,t)\) scores negative and each self-pair scores positive.
4.  [Compare with an extreme.]{.wex-op} \(K(s,u)=(0.5)(1.5)+(-0.5)(-1.5)=1.5\): the \(A\)-heavy strings \(s\) and \(u\) align strongly, since both would push the model toward emitting more \(A\).
5.  [Inspect the information matrix.]{.wex-op} The single-symbol Fisher information under the uniform model is \(I_1=\begin{pmatrix}0.25 & -0.25\\ -0.25 & 0.25\end{pmatrix}\), with \(\det I_1=0\). It is singular because the two counts satisfy \(n_A+n_B=n\), so one score coordinate is redundant, which is exactly why \(I\) is set to the identity here.
...
1.  [computation]{.ex-tag} Verify the two-element counterexample of the opening section: for \(P(x_1,x_2)=P(x_2,x_1)=\tfrac12\) and zeros on the diagonal, write down the \(2\times 2\) Gram matrix and confirm its eigenvalues are \(\pm\tfrac12\). Then show that if instead \(P(x_1,x_1)=P(x_2,x_2)=\tfrac12\) and the off-diagonal is zero, the distribution is a P-kernel.
2.  [computation]{.ex-tag} In the 2-state HMM of the worked example, recompute \(\kappa(s,t)\) for \(s=\texttt{AA}\) and \(t=\texttt{AA}\) both by enumerating the four hidden paths and by the forward recursion, and confirm the two agree. Is \(\kappa(\texttt{AA},\texttt{AA})\) larger or smaller than \(\kappa(\texttt{AB},\texttt{AA})\), and why?
3.  [proof]{.ex-tag} Show that the fixed-length marginalization kernel with independent hidden states, \(\kappa(s,t)=\sum_{h\in A^n}\prod_i P(s_i\mid h_i)P(t_i\mid h_i)P(h_i)\), factorizes as \(\prod_{i=1}^n\big(\sum_{a}P(s_i\mid a)P(t_i\mid a)P(a)\big)\). Hint: apply the distributive law one position at a time, or argue by induction on \(n\).
4.  [proof]{.ex-tag} Prove that the sum \(\sum_{s,t\in\Sigma^n}\kappa(s,t)\) of the fixed-length kernel over all string pairs equals \(1\). Hint: exchange the order of summation so the sum over \((s_i,t_i)\) sits inside the product, and use that the emission normalization collapses each position's factor to \(1\).
5.  [computation]{.ex-tag} Take the pair HMM recursion with a single emitting state \(a\) beyond \(a_I\), self-transition probability \(1\), and emissions \(P((\sigma,\sigma)\mid a)=m\) for a match, \(P((\sigma,\sigma')\mid a)=\mu\) for a mismatch, and \(P((\sigma,\varepsilon)\mid a)=P((\varepsilon,\sigma)\mid a)=d\) for a gap. Fill the \(\kappa_{i,j}\) table by hand for \(s=\texttt{AB}\), \(t=\texttt{A}\) and read off \(\kappa(s,t)\) in terms of \(m,\mu,d\).
6.  [computation]{.ex-tag} For the binary hidden tree with a single internal root and two leaves, parameters \(\epsilon,\delta,\tau\) as in the example, write \(\kappa(s,t)\) explicitly for the leaf pair \(s_1=t_1=1\) on the first leaf and \(s_2=t_2=1\) on the second, by carrying out the root sum \(\sum_a P_M(a)\prod_j\sum_b P(b\mid a)\kappa_{\mathrm{ch}_j,b}\). Hint: the leaf messages are indicators \([\,s_i=t_i=b\,]\).
7.  [computation]{.ex-tag} Let \(P_\theta(x)=e^{-\theta}\theta^x/x!\) be the Poisson model on \(x\in\mathbb N_0\) with scalar \(\theta\gt 0\). Compute the Fisher score \(g(\theta,x)=\partial_\theta\log P_\theta(x)\), the Fisher information \(I(\theta)=\mathbb E[g(\theta,x)^2]\), and the whitened one-dimensional Fisher kernel \(K(x,x')=g(\theta,x)I(\theta)^{-1}g(\theta,x')\). Which pairs \((x,x')\) receive a positive value at \(\theta=2\)?
8.  [challenge]{.ex-tag} For the 2-symbol iid model of the worked example, take the generative model to be a first-order Markov chain over \(k\)-mers with transition weights \(a_{u\to\sigma}\) normalized to \(p_{u\to\sigma}=a_{u\to\sigma}/\sum_{\sigma'}a_{u\to\sigma'}\). Differentiate the log-likelihood at the flat setting \(a_{u\to\sigma}=|\Sigma|^{-1}\) to show the score coordinate for \(u\to\sigma\) is \(|\{(v_1,v_2):s=v_1 u\sigma v_2\}|-|\Sigma|^{-1}|\{(v_1,v_2):s=v_1 u v_2\}|\), and conclude that up to the centering term the Fisher kernel is the \((k{+}1)\)-spectrum kernel. Hint: the log-likelihood is a sum over positions of \(\log p_{s(j:j+k-1)\to s_{j+k}}\); see [[ch:string-kernels|the string-kernels chapter]].
