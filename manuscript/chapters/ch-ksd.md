...
Let \(k\) be a positive definite kernel on \(\mathbb{R}^d\) with RKHS \(\mathcal H\), and let \(\mathcal H^d = \mathcal H \times \cdots \times \mathcal H\) be the vector-valued RKHS of fields \(f = (f_1,\dots,f_d)\) with \(f_i \in \mathcal H\) and squared norm \(\lVert f\rVert_{\mathcal H^d}^2 = \sum_i \lVert f_i\rVert_{\mathcal H}^2\). Taking \(\mathcal F\) to be its unit ball gives the kernel Stein discrepancy.
